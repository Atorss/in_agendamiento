...
class WhatsappWebhookController(http.Controller):
    ...
    def receive_message(self, **_kwargs):
        ...
        # Gate de suscripción: el agente WhatsApp solo opera si el plan del
        # tenant tiene la feature habilitada (en vivo). Si no, no procesamos
        # ni gastamos tokens IA. skip_send=True para que n8n NO intente
        # responderle al cliente (evita 400 de Meta).
        has_whatsapp = request.env['in_agenda.suscripcion'].sudo()._company_has_feature(
            company, 'whatsapp')
        if not has_whatsapp:
            _logger.info(
                'WhatsApp deshabilitado para company %s (sin feature en plan)',
                company.id)
