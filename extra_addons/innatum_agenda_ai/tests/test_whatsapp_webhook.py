...
class TestWhatsappWebhook(HttpCase):
    ...
    def setUp(self):
        ...
        self.tmpl = self.env.ref('innatum_agenda_ai.vertical_template_odontologia')
        self.profile = self.env['innatum.business.profile'].create({
            'company_id': self.company.id,
            'vertical_template_id': self.tmpl.id,
        })
        # Suscripción con el add-on WhatsApp VIGENTE: el webhook gatea por la
        # feature 'whatsapp' antes de procesar. Sin add-on vigente, el agente
        # queda deshabilitado para el tenant (comportamiento correcto del gate).
        plan = self.env['in_agenda.plan'].create({
            'name': 'Test Plan WA', 'code': 'TEST_WA',
        })
        susc = self.env['in_agenda.suscripcion'].create({
            'company_id': self.company.id,
            'plan_id': plan.id,
            'fecha_fin': '2099-12-31',
            'state': 'active',
        })
        self.env['in_agenda.suscripcion.addon'].create({
            'suscripcion_id': susc.id,
            'addon_id': self.env.ref('innatum_agenda_planes.addon_whatsapp').id,
            'fecha_inicio': '2020-01-01',
        })
        ICP = self.env['ir.config_parameter'].sudo()
        ICP.set_param('innatum_ai_core.n8n_shared_secret', 'fase1a-test-secret')
        # Modo echo (sin IA) para los tests de transporte del webhook.
        ICP.set_param('innatum_ai_core.agent_enabled', '0')
